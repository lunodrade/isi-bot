...
Glob('gitlab/*.js', function(er, files) {
  files.forEach(filename => {
    let reqName = filename.replace('gitlab/', '').replace('.js', '');
...
async function sendEmbedToDiscord(
  ...
  try {
    var embed = GitlabEvents[fnEvent][fnAction + 'Embed'](
      Discord.RichEmbed,
      json,
      avatar
    );
...
app.post('/gitlab', function(req, res) {
  ...
  if (
    ...
    //TODO: verificar se tem commit[] pq se não pode ser o primeiro push do projeto mesmo
